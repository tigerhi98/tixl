...
internal sealed class Video2DPointScanner : Instance<Video2DPointScanner>
{
    public Video2DPointScanner()
    {
        ...
        public readonly InputSlot<T3.Core.DataTypes.Texture2D> VideoIn = new InputSlot<T3.Core.DataTypes.Texture2D>();

        [Input(Guid = "1d11b92d-bf80-4f4f-9061-17fda30fb895")]
        public readonly InputSlot<int> PixelCount = new InputSlot<int>();

        [Input(Guid = "bcfd87c4-6706-4447-8bbd-c069b6ebae70")]
        public readonly InputSlot<float> PixelBrightness = new InputSlot<float>();
        ...
        public readonly InputSlot<bool> ScanTrigger = new InputSlot<bool>();

        [Input(Guid = "b123fade-921a-46c2-98bd-b4e78a8d4c30")]
        public readonly InputSlot<float> ScanIntervallum = new InputSlot<float>();

        [Input(Guid = "3cfd6232-1506-4ba1-bcc1-1c0839a7de64")]
        public readonly InputSlot<bool> ResetScan = new InputSlot<bool>();
        ...
        public readonly InputSlot<bool> Save = new InputSlot<bool>();

        [Input(Guid = "572bfd98-9746-4d34-b0cd-e22a4c6d75fd")]
        public readonly InputSlot<bool> Load = new InputSlot<bool>();

        [Input(Guid = "91472e9f-5168-432b-92cd-ec826b72ffd4")]
        public readonly InputSlot<string> CalibrationPath = new InputSlot<string>();

        [Input(Guid = "5b168a97-851b-4928-93f9-6a8a729fed3b")]
        public readonly InputSlot<bool> LoadCalibration = new InputSlot<bool>();

        [Input(Guid = "4a0c8d1e-1b2c-4f5a-8b9c-0d1e2f3a4b5c")]
        public readonly InputSlot<bool> ApplyCorrection = new InputSlot<bool>();

        [Input(Guid = "58d8e589-1536-4728-8d46-de2311ce61d4")]
        public readonly InputSlot<bool> TestFullMode = new InputSlot<bool>();

        [Input(Guid = "9ac8647b-0305-4bd3-a1fc-b47fdc46c297")]
        public readonly InputSlot<bool> TestPixelMode = new InputSlot<bool>();

        [Input(Guid = "89d26124-b476-443d-bdd0-82c76c0f3789")]
        public readonly InputSlot<bool> DebugMode = new InputSlot<bool>();
    ...
    public readonly Slot<Texture2D> DebugTexture = new Slot<Texture2D>();
    ...
    private static readonly JsonSerializerOptions _jsonOptions = new JsonSerializerOptions { WriteIndented = true, IncludeFields = true };
    ...
    private void Update(EvaluationContext context)
    {
        ...
        if (videoTex == null) { HandleNoVideoInput(); return; }

        using var rawFrame = ConvertTextureToMat(videoTex);
        if (rawFrame == null) { HandleVideoConversionFailure(); return; }
        ...
        if (_isCalibrated && ApplyCorrection.GetValue(context))
        {
            ...
            for(int r=0; r < 3; r++) for(int c=0; c < 3; c++) cameraMat.Set(r, c, _cameraMatrix[r,c]);
            for(int i=0; i < _distCoeffs.Length; i++) distMat.Set(0, i, _distCoeffs[i]);
        ...
        var lightOutputPoints = new List<Point>();
        ...
        else if (testPixelMode)
        {
            ...
            lightOutputPoints = GenerateLightOutput(pixelCount, _currentLedIndex, brightness, false);
        ...
        else if (_isProcessing)
        {
            ...
            lightOutputPoints = GenerateLightOutput(pixelCount, nextLedToScan, brightness, false);
            DrawPointsOnFrame(debugFrame, _currentLed2DPositions, new Scalar(0,255,255));
        }
        else // Idle
        {
            lightOutputPoints = GenerateLightOutput(pixelCount, -1, 0, false);
    ...
    private void HandleInputsAndTriggers(EvaluationContext context)
    {
        ...
        if (MathUtils.WasTriggered(Load.GetValue(context), ref _loadTriggered))
        {
            ...
            if(string.IsNullOrEmpty(path)) { Log.Warning("Load failed: File path is empty.", this); return; }
            ...
            try {
                ...
                _currentLed2DPositions = Enumerable.Repeat(Vector2.Zero, currentPixelCount).ToList();
            ...
            } catch (Exception e) { Log.Error($"Load failed: {e.Message}", this); }
        }
        
        if (MathUtils.WasTriggered(Save.GetValue(context), ref _saveTriggered))
        {
            var path = FilePath.GetValue(context);
            if(string.IsNullOrEmpty(path)) { Log.Warning("Save failed: File path is empty.", this); return; }
            ...
            } catch (Exception e) { Log.Error($"Save failed: {e.Message}", this); }
        }
    ...
    {
        _currentLedIndex = -1;
        _currentLed2DPositions = Enumerable.Repeat(Vector2.Zero, pixelCount).ToList();
    }
    ...
    public static bool LoadCalibrationData(string filepath, out double[,] cameraMatrix, out double[] distCoeffs)
    {
        ...
        try {
            ...
            for(int r=0; r<3; r++) { var v = lines[r+1].Split(','); for(int c=0; c<3; c++) cameraMatrix[r,c] = double.Parse(v[c], CultureInfo.InvariantCulture); }
            var dv = lines[5].Split(',');
            distCoeffs = new double[dv.Length];
            for(int i=0; i<dv.Length; i++) distCoeffs[i] = double.Parse(dv[i], CultureInfo.InvariantCulture);
            return true;
        }
        catch (Exception e) { Log.Error($"Failed to load calibration file '{filepath}': {e.Message}"); return false; }
    }
    
    private void HandleNoVideoInput() { UpdateGpuBufferWithPoints(ref _pixelOutputBuffer, new List<Point>()); UpdateGpuBufferWithPoints(ref _scannedPointsBuffer, new List<Point>()); PixelOutput.Value = _pixelOutputBuffer; ScannedPoints2D.Value = _scannedPointsBuffer; }
    private void HandleVideoConversionFailure() { UpdateGpuBufferWithPoints(ref _pixelOutputBuffer, new List<Point>()); UpdateGpuBufferWithPoints(ref _scannedPointsBuffer, new List<Point>()); PixelOutput.Value = _pixelOutputBuffer; ScannedPoints2D.Value = _scannedPointsBuffer; }
        ...
        if (findAll)
        {
            foreach (var c in contours)
            {
                if (Cv2.ContourArea(c) > 2)
                {
                    var m = Cv2.Moments(c);
                    if (m.M00 != 0)
                        points.Add(new Point { Position = new Vector3(((float)(m.M10 / m.M00) / frame.Width - 0.5f) * 2f, ((float)(m.M01 / m.M00) / frame.Height - 0.5f) * -2f, 0), F1 = 1 });
                }
            }
        }
        else
    ...
    private void DrawPointsOnFrame(Mat frame, List<Vector2> points, Scalar color) { foreach (var v in points.Where(v => v != Vector2.Zero)) Cv2.Circle(frame, new OpenCvSharp.Point((int)((v.X / 2f + 0.5f) * frame.Width), (int)((v.Y / -2f + 0.5f) * frame.Height)), 5, color, -1); }
    private void AddDebugText(Mat frame, string text, Scalar color) { Cv2.PutText(frame, text, new OpenCvSharp.Point(10, 30), HersheyFonts.HersheySimplex, 0.7, color, 2, LineTypes.AntiAlias); }
    private void UpdateGpuBufferWithPoints(ref BufferWithViews buffer, List<Point> list) { int num = list?.Count ?? 0; if (num != (buffer?.Buffer.Description.SizeInBytes / Point.Stride ?? 0)) { buffer?.Dispose(); buffer = null; } if (num == 0) return; var array = list.ToArray(); if (buffer == null) { buffer = new BufferWithViews(); ResourceManager.SetupStructuredBuffer(array, Point.Stride * num, Point.Stride, ref buffer.Buffer); ResourceManager.CreateStructuredBufferSrv(buffer.Buffer, ref buffer.Srv); ResourceManager.CreateStructuredBufferUav(buffer.Buffer, UnorderedAccessViewBufferFlags.None, ref buffer.Uav); } else { ResourceManager.Device.ImmediateContext.UpdateSubresource(array, buffer.Buffer); } }
    ...
    private static Mat ConvertTextureToMat(Texture2D tex)
    {
        ...
        var box = context.MapSubresource(stageTex, 0, MapMode.Read, SharpDX.Direct3D11.MapFlags.None);
    ...
    private void UploadMatToTexture(Mat mat, ref Texture2D tex)
    {
        try
        {
            ...
            {
                ResourceManager.Device.ImmediateContext.UpdateSubresource(new DataBox(bmpData.Scan0, bmpData.Stride, 0), tex, 0);
            }
    ...
    protected override void Dispose(bool disposing) { if (!disposing) return; _pixelOutputBuffer?.Dispose(); _scannedPointsBuffer?.Dispose(); _debugTexture?.Dispose(); }
