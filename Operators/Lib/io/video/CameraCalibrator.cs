...
namespace Lib.io.video
{
    ...
    {
        public enum Modes { Passthrough, Calibration, LensCorrected }

        public enum UndistortMethods { Undistort, Remap }
        ...
        private void Update(EvaluationContext context)
        {
            ...
            var patternSize = new OpenCvSharp.Size(patternSizeVec2.X, patternSizeVec2.Y);
            var displayResolution = new OpenCvSharp.Size(displayResolutionInt2.X, displayResolutionInt2.Y);

            bool needsRegen = _checkerboardTexture == null || patternSize != _lastPatternSize || displayResolution != _lastDisplayResolution || border != _lastBorderSize;
            if (needsRegen)
            {
        ...
        private Mat ProcessLensCorrection(Mat frame, EvaluationContext context)
        {
            ...
            using var newCameraMat = Cv2.GetOptimalNewCameraMatrix(cameraMat, distMat, _frameSize, alpha, _frameSize, out Rect roi, false);
            if (newCameraMat == null || newCameraMat.Empty())
            {
            ...
            try
            {
                switch (undistortMethod)
                {
                    ...
                    case UndistortMethods.Remap:
                    {
                        ...
                        using var tempFrame = new Mat();
                        Cv2.Remap(frame, tempFrame, mapX, mapY, InterpolationFlags.Linear);
                        if (roi.Width > 0 && roi.Height > 0)
                        {
                            using var cropped = new Mat(tempFrame, roi);
                            processedFrame = cropped.Clone();
        ...
        {
            var boardSize = new OpenCvSharp.Size(ChessboardSize.GetValue(context).X, ChessboardSize.GetValue(context).Y);
            ...
            if (Cv2.FindChessboardCorners(gray, boardSize, out Point2f[] corners))
            {
                ...
                StatusMessage.Value = $"Pattern found! Captured {_calibrationImagePoints.Count}/{MIN_CALIBRATION_SAMPLES}. Press Capture.";
            ...
            {
                StatusMessage.Value = $"Searching for {boardSize.Width}x{boardSize.Height} board... Captured {_calibrationImagePoints.Count}/{MIN_CALIBRATION_SAMPLES}.";
            }
        ...
        {
            if (_calibrationImagePoints.Count < MIN_CALIBRATION_SAMPLES)
            {
                StatusMessage.Value = $"Need at least {MIN_CALIBRATION_SAMPLES} samples, have {_calibrationImagePoints.Count}.";
            ...
            {
                Log.Warning("[Calibrator] All distortion coefficients are zero. Lens may have minimal distortion or calibration data may lack diversity. Consider capturing more varied images (different angles, edge coverage).", this);
                StatusMessage.Value = $"Calibration complete! RMS error: {rms:F4}. Warning: No significant distortion detected.";
            }
            else
            {
                StatusMessage.Value = $"Calibration complete! RMS error: {rms:F4}. Distortion coeffs: {string.Join(", ", _distCoeffs.Select(x => x.ToString("F6")))}";
            }

            _isCalibrated = true;
        }

        private void GenerateCheckerboard(OpenCvSharp.Size patternSize, OpenCvSharp.Size imageSize, int border)
        {
            ...
            {
                g.Clear(System.Drawing.Color.White);  // Explicitly qualified Color
        ...
        private void HandleTriggers(EvaluationContext context)
        {
            ...
            {
                _calibrationImagePoints.Clear(); _calibrationObjectPoints.Clear();
                _isCalibrated = false;
        ...
        private Point3f[] Create3DChessboardCorners(OpenCvSharp.Size boardSize, float squareInMm)
        {
        ...
        private void SaveCalibrationData(string filepath)
        {
            ...
            try
            {
                ...
                for (int r = 0; r < 3; r++) lines.Add($"{_cameraMatrix[r,0]},{_cameraMatrix[r,1]},{_cameraMatrix[r,2]}");
            ...
            catch (Exception e) { Log.Error($"[Calibrator] Failed to save calibration: {e.Message}", this); }
        }
        #endregion
        ...
        {
            cameraMatrix = null; distCoeffs = null;
            if (!File.Exists(filepath)) { Log.Warning($"[Calibration] File not found: {filepath}"); return false; }
            try
            {
                ...
                for(int r=0; r < 3; r++)
                {
                    ...
                    for(int c=0; c < 3; c++) cameraMatrix[r,c] = double.Parse(vals[c]);
                }
                var distVals = lines[5].Split(',');
                distCoeffs = new double[distVals.Length];
                for(int i=0; i < distVals.Length; i++) distCoeffs[i] = double.Parse(distVals[i]);
                return true;
            }
            catch (Exception e)
            {
        ...
        public static Mat ConvertTextureToMat(T3.Core.DataTypes.Texture2D t3Texture)
        {
            ...
            var stagingDesc = new Texture2DDescription { Width = desc.Width, Height = desc.Height, MipLevels = 1, ArraySize = 1, Format = desc.Format, SampleDescription = new SharpDX.DXGI.SampleDescription(1, 0), Usage = ResourceUsage.Staging, BindFlags = BindFlags.None, CpuAccessFlags = CpuAccessFlags.Read, OptionFlags = ResourceOptionFlags.None };
            ...
            var dataBox = deviceContext.MapSubresource(stagingTexture, 0, MapMode.Read, SharpDX.Direct3D11.MapFlags.None);
        ...
        public static void UploadMatToTexture(Mat mat, ref T3.Core.DataTypes.Texture2D texture)
        {
        ...
        public static void UploadBitmapToTexture(Bitmap bmp, ref T3.Core.DataTypes.Texture2D texture)
        {
            if (bmp == null) return;
            try
            {
                ...
                {
                    texture?.Dispose();
                    var texDesc = new Texture2DDescription { Width = bmp.Width, Height = bmp.Height, MipLevels = 1, ArraySize = 1, Format = SharpDX.DXGI.Format.B8G8R8A8_UNorm, SampleDescription = new SharpDX.DXGI.SampleDescription(1, 0), Usage = ResourceUsage.Default, BindFlags = BindFlags.ShaderResource };
                    var dataRectangle = new DataRectangle(bmpData.Scan0, bmpData.Stride);
                    texture = new T3.Core.DataTypes.Texture2D(new SharpDX.Direct3D11.Texture2D(ResourceManager.Device, texDesc, new[] { dataRectangle }));
                }
                else
                {
                    ResourceManager.Device.ImmediateContext.UpdateSubresource(new DataBox(bmpData.Scan0, bmpData.Stride, 0), texture, 0);
                }
                bmp.UnlockBits(bmpData);
            }
            catch (Exception e) { Log.Error($"Failed to upload texture: {e.Message}"); }
        }
        #endregion
        ...
        private const int MIN_CALIBRATION_SAMPLES = 15;
        ...
        private OpenCvSharp.Size _lastPatternSize, _lastDisplayResolution;
        private int _lastBorderSize;
        private double[,] _cameraMatrix = new double[3, 3];
        private double[] _distCoeffs = new double[8];
        private List<Point2f[]> _calibrationImagePoints = new List<Point2f[]>();
        private List<Point3f[]> _calibrationObjectPoints = new List<Point3f[]>();
        private T3.Core.DataTypes.Texture2D _textureOut, _checkerboardTexture;

        [Input(Guid = "A3F7D2B1-9C8E-4A6D-8F1C-2B9E3D7A5C2E")] public readonly InputSlot<Texture2D> TextureIn = new();
        [Input(Guid = "B4C8E3F2-0D7F-4B5E-9C2D-3C0F4E8B6A3D", MappedType = typeof(Modes))] public readonly InputSlot<int> Mode = new((int)Modes.Passthrough);
        [Input(Guid = "C5D9F4A3-1E8C-4C6F-0D3E-4D1F5F9C7B4E")] public readonly InputSlot<Int2> ChessboardSize = new(new Int2(7,6));
        [Input(Guid = "D6E0F5B4-2F9D-4D7F-1E4F-5E2F6F0D8C5F")] public readonly InputSlot<Int2> DisplayResolution = new(new Int2(1920, 1080));
        [Input(Guid = "E7F1F6C5-3F0E-4E8F-2F5F-6F3F7F1E9D6F")] public readonly InputSlot<int> BorderInSquares = new(1);
        [Input(Guid = "F8F2F7D6-4F1F-4F9F-3F6F-7F4F8F2F0E7F")] public readonly InputSlot<float> SquareInMm = new(25.0f);
        [Input(Guid = "A9F3F8E7-5F2F-4F0F-4F7F-8F5F9F3F1F8F")] public readonly InputSlot<bool> CaptureImage = new();
        [Input(Guid = "B0F4F9F8-6F3F-4F1F-5F8F-9F6F0F4F2F9F")] public readonly InputSlot<bool> Calibrate = new();
        [Input(Guid = "C1F5F0F9-7F4F-4F2F-6F9F-0F7F1F5F3F0F")] public readonly InputSlot<bool> Reset = new();
        [Input(Guid = "D2F6F1F0-8F5F-4F3F-7F0F-1F8F2F6F4F1F")] public readonly InputSlot<string> FilePath = new("calibration.dat");
        [Input(Guid = "E3F7F2F1-9F6F-4F4F-8F1F-2F9F3F7F5F2F")] public readonly InputSlot<bool> Save = new();
        [Input(Guid = "F4F8F3F2-0F7F-4F5F-9F2F-3F0F4F8F6F3F")] public readonly InputSlot<bool> Load = new();
        [Input(Guid = "1A2B3C4D-5E6F-4A1B-8C9D-0E1F2A3B4C5D", MappedType = typeof(UndistortMethods))] public readonly InputSlot<int> UndistortMethod = new((int)UndistortMethods.Undistort);
        [Input(Guid = "2A3B4C5D-6E7F-4B2C-9DAE-1F2A3B4C5D6E")] public readonly InputSlot<float> Alpha = new(1.0f);

        [Output(Guid = "3A4B5C6D-7E8F-4C3D-AFBF-2F3A4B5C6D7E")] public readonly Slot<Texture2D> CheckerboardTexture = new();
        [Output(Guid = "A5F9F4F3-1F8F-4F6F-0F3F-4F1F5F9F7F4F")] public readonly Slot<Texture2D> TextureOut = new();
        [Output(Guid = "B6F0F5F4-2F9F-4F7F-1F4F-5F2F6F0F8F5F")] public readonly Slot<bool> IsCalibrated = new();
        [Output(Guid = "C7F1F6F5-3F0F-4F8F-2F5F-6F3F7F1F9F6F")] public readonly Slot<string> StatusMessage = new();
    }
}